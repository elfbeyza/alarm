...
const alarmSound = new Audio("./alarm-auto.mp3");
...
const displayTimer = () => {
    const date = new Date();
    const currentTime = date.toLocaleTimeString("tr-TR", { hour12: false });
...
const toggleAlarm = (alarm) => {
    ...
    if (alarm.isActive) {
        const currentTime = new Date().toLocaleTimeString("tr-TR", { hour12: false }).slice(0, 5);
...
setAlarm.addEventListener("click", () => {
    ...
    if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
        alert("Geçersiz saat ya da dakika aralığı girdiniz!");
        return;
    }
